-- ---------------------------------------------------------------------
-- Phase 3V — save_provider_profile() says what it refused, and why
--
-- phase3r put a trigger on all three party tables so a row may only exist for
-- the role that owns it. The seeker half of M8 was updated to match:
-- save_seeker_profile() checks the role itself and raises a sentence, so a
-- client is told "This account is not a family account." rather than handed a
-- constraint violation to translate. save_provider_profile() predates phase3r
-- and was never given the same check.
--
-- What that costs is only visible when the guard actually fires — an account
-- whose profiles.role is not 'provider' calling PUT /api/v1/providers/me. The
-- trigger raises check_violation (23514), which is not P0001, so the service
-- answered 500 with a raw Postgres message: a refusal that is correct,
-- reported as a server fault.
--
-- Two halves, and both are wanted:
--   * here, so the refusal is a sentence whatever calls it, psql included;
--   * in providers.service.ts, which now maps 23514 the way seekers.service
--     has all along, for the case where the trigger is reached first.
--
-- Nothing else changes. The body below is phase3o's, with the check added
-- after the provider_type validation — reproduced whole because CREATE OR
-- REPLACE FUNCTION has no way to amend one.
--
...
begin
  ...
  -- phase3r's trigger refuses a providers row for an account that is not a
  -- provider. Checked here too so the refusal is a sentence rather than a
  -- constraint violation the client has to translate. Mirrors the same check
  -- in save_seeker_profile(), which phase3s added and this was never given.
  --
  -- A missing profiles row is left alone, which is the call phase3r's trigger
  -- already makes: the foreign key on user_id decides whether the account
  -- exists, and inventing a refusal here would break an order of writes that
  -- works today.
  select role into v_role from public.profiles where id = v_user;
  if v_role is not null and v_role <> 'provider' then
    raise exception 'This account is a %, so it cannot have a coach listing.', v_role
      using hint = 'Changing your account type is a different action, and not always allowed.';
  end if;
